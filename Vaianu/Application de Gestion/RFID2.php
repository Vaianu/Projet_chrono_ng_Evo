...
$heure_depart = $requete->fetch();
$heure_recupRFIDStation = DiminuerDeCinqMinutes($heure_depart['h_depart']); // Diminution heure départ pour que les stations récupèrent les numéros RFID 5 minutes avant le départ
$heure = date('H:i:s'); // heure courante pour ensuite comparaison avec heure diminuer

/*----------------# Enregistrement des temps passage des coureurs dans la base de données #-------------------------*/
if (isset($_GET['_numRFID']) && isset($_GET['_heurePass']) && isset($_GET['_numSta'])) // si toutes les variables sont présents dans l'URL
{
...
else if ($heure >= $heure_recupRFIDStation) // si heure pour récupérer l'heure courante et les numéros RFID par les stations est atteint ou dépassé
{
